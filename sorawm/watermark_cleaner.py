...
if __name__ == "__main__":
    ...
    def _clip_rect(x0, y0, x1, y1, w_img, h_img):
        x0 = max(0, min(x0, w_img-1)); x1 = max(0, min(x1, w_img))
        y0 = max(0, min(y0, h_img-1)); y1 = max(0, min(y1, h_img))
        if x1 <= x0: x1 = x0 + 1
        if y1 <= y0: y1 = y0 + 1
        return x0, y0, x1, y1
    ...
    def process_and_show(frame, idx):
        img = frame.copy()
        bw, best, contours, region_boxes, prev_roi_box = detect_flower_like(img, state["bbox"])
        ...
        if best is not None:
            ...
            # 若仍未检测，维持上一状态
            cv2.putText(vis, "No detection (kept last state)", (12, 28),
        ...
        cv2.putText(panel, f"Frame {idx} | {title}", (12, 28),
